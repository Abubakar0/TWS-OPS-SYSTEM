...
const RUN_CREATE_FLOW =
  process.env.ACCOUNT_INVOICE_REGRESSION_CREATE === "true";
...
function buildInvoicePayload(accountName) {
  return {
    ...
    invoiceMonth: new Date().toLocaleDateString("en-CA").slice(0, 7),
...
async function main() {
  ...
  if (!account?.id) {
    throw new Error(
      "Could not find an account to run the invoice regression against.",
    );
  }
  ...
  const hunterSummaryBlocked = await request(
    `/accounts/${account.id}/summary`,
    {
      token: hunterToken,
  ...
  const hunterInvoiceBlocked = await request(
    `/accounts/${account.id}/invoices`,
    {
      method: "POST",
      token: hunterToken,
    ...
    if (createInvoice.ok) {
      ...
      pushManual(
        ...
        invoice?.primaryPayment?.bankName === payload.primaryPayment.bankName &&
          invoice?.alternatePayment?.bankName ===
            payload.alternatePayment.bankName,
        JSON.stringify({
      ...
      const refreshedSummary = await request(
        `/accounts/${account.id}/summary`,
        {
          token: adminToken,
          expectedStatuses: [200],
      ...
      if (refreshedSummary.ok) {
        const invoiceFound = (refreshedSummary.body?.invoices || []).some(
          (entry) =>
            entry.id === invoice?.id ||
            entry.invoiceCode === invoice?.invoiceCode,
        );
        pushManual(
          ...
          invoiceFound
            ...
            : "Invoice not found in summary list.",
        );
      }
    }
  }
