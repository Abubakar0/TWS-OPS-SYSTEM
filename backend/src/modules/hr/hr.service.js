...
const getDatePartInTimezone = (value, part) => {
  const formatValue = part === "year" ? "numeric" : "2-digit";
  const token = new Intl.DateTimeFormat("en-US", {
    timeZone: BUSINESS_TIMEZONE,
    [part]: formatValue,
  })
    .formatToParts(value instanceof Date ? value : new Date(value))
    .find((entry) => entry.type === part);

  return token?.value || "";
};

const getMonthDayFromDateValue = (value) => {
...
const getCurrentMonthDay = () =>
  `${getDatePartInTimezone(new Date(), "month")}-${getDatePartInTimezone(
    new Date(),
    "day",
  )}`;
...
const getCurrentDateIso = () =>
  [
    getDatePartInTimezone(new Date(), "year"),
    getDatePartInTimezone(new Date(), "month"),
    getDatePartInTimezone(new Date(), "day"),
  ].join("-");
...
const getCurrentYear = () =>
  Number.parseInt(
    new Intl.DateTimeFormat("en-US", {
      ...
      .find((entry) => entry.type === "year")?.value || "0",
    10,
...
const buildEmployeeFilters = (query = {}) => {
  ...
  if (query.excludeSuperAdmin) {
    where.push(
      `NOT (COALESCE(u.roles, jsonb_build_array(u.role::text)) @> '["super_admin"]'::jsonb)`,
    );
  }
...
const updateEmployee = async (viewer, id, payload = {}) => {
  ...
  if (payload.dateOfBirth !== undefined)
    add("date_of_birth", toDate(payload.dateOfBirth));
...
const listExpenses = async (viewer, query = {}) => {
  ...
  if (query.dateFrom) {
    params.push(query.dateFrom);
    where.push(`x.expense_date >= $${params.length}::date`);
  }

  if (query.dateTo) {
    params.push(query.dateTo);
    where.push(`x.expense_date < ($${params.length}::date + INTERVAL '1 day')`);
  }
...
const getHrDashboard = async (viewer, query = {}) => {
  ...
  const dateFrom = toDate(query.dateFrom) || todayDate;
  ...
  const [
    ...
    pool.query(
      `
        SELECT
          ...
          (SELECT COUNT(*)::int FROM hr_leave_requests WHERE status = 'APPROVED' AND $1::date BETWEEN start_date AND end_date) AS "onLeave",
          (SELECT COUNT(*)::int FROM hr_leave_requests WHERE status = 'PENDING') AS "pendingLeaves",
          (SELECT COUNT(*)::int FROM hr_expenses WHERE status = 'SUBMITTED') AS "pendingExpenses",
          COALESCE((
            SELECT SUM(payroll.net_salary)
            FROM hr_payroll payroll
            WHERE date_trunc('month', payroll.payroll_month) = date_trunc('month', $1::date)
          ), 0)::numeric(10,2) AS "monthlySalaryCost"
      `,
      [todayDate],
    ),
    pool.query(
    ...
    pool.query(
      `
        ...
        JOIN employee_profiles e ON e.id = l.employee_profile_id
        JOIN users u ON u.id = e.user_id
        WHERE l.status = 'APPROVED' AND l.start_date >= $1::date
    ...
    pool.query(
      `
        SELECT
          ...
          EXTRACT(YEAR FROM age($1::date, e.joining_date))::int AS years
        FROM employee_profiles e
        JOIN users u ON u.id = e.user_id
    ...
    pool.query(
      `
        SELECT
          ...
          e.designation,
          e.joining_date AS "eventDate",
          (EXTRACT(YEAR FROM age($1::date, e.joining_date))::int + 1) AS years
        FROM employee_profiles e
        JOIN users u ON u.id = e.user_id
        WHERE e.joining_date IS NOT NULL
          AND to_char(e.joining_date, 'MM-DD') > to_char($1::date, 'MM-DD')
...
const getMyHr = async (viewer) => {
  ...
  const employeeBirthday = getMonthDayFromDateValue(employee.dateOfBirth);
  const birthdayToday = employeeBirthday === getCurrentMonthDay();
